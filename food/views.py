...
from .forms import FoodCommentForm, FoodForm
from .models import Food, FoodComment, FoodCommentDislike, FoodCommentLike, Restaurant
...
class FoodCommentCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse("food:food_detail", kwargs={"pk": self.kwargs.get("pk")})
...
class FoodCommentUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse("food:food_detail", kwargs={"pk": self.object.restaurant.pk})
...
def food_comment_like_view(request, comment_pk):
    ...
    if like_comment and user == like_comment.user:
        ...
        return redirect("food:food_detail", pk=food_pk)
...
def food_comment_reply_view(request, parent_comment_pk):
    ...
    if request.method == "POST":
        ...
        if not text_replay:
            ...
            return redirect("food:food_detail", pk=parent_comment.food.pk)
